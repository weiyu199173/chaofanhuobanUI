...
const MeScreen = ({ onCreateAgent }: { onCreateAgent: () => void }) => {
  const agents: Agent[] = [
    { id: '1', name: 'Echo-01', avatar: 'https://picsum.photos/seed/echo/100/100', syncRate: 85, status: 'active', traits: ['冷静', '严谨'] },
    { id: '2', name: 'Nova', avatar: 'https://picsum.photos/seed/nova2/100/100', syncRate: 42, status: 'training', traits: ['幽默', '感性'] },
  ];
                  ...
                  <div className="px-6 pb-6 -mt-10 relative">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex justify-between gap-3">
                        <button className="flex-1 py-2 bg-on-surface text-background text-[10px] font-bold uppercase tracking-widest rounded-lg active:scale-95 transition-all">管理</button>
                        <button className="flex-1 py-2 bg-surface-container-highest text-on-surface text-[10px] font-bold uppercase tracking-widest rounded-lg border border-white/5 active:scale-95 transition-all">互动</button>
                      </div>
                    </div>
                  </div>
                </motion.div>
...
const ContactsScreen = ({ onChatClick }: { onChatClick: (id: string) => void }) => {
  const [searchQuery, setSearchQuery] = useState('');
            ...
            >
              ...
              <div className="px-8 pb-8 -mt-12 text-center relative">
                ...
                <div className="grid grid-cols-2 gap-4 mt-8">
                  ...
                  <button className="bg-surface-container-highest text-on-surface py-3 rounded-2xl font-bold flex items-center justify-center gap-2 border border-white/5 active:scale-95 transition-all text-sm">
                    <User size={18} /> 详细信息
                  </button>
                </div>
              </div>
...
export default function App() {
  ...
  // Restore session or something?
  useEffect(() => {
    // For demo purposes, we stay on login if not clicked
  }, []);
      ...
      <AnimatePresence>
        {currentView === 'main' && (
          ...
          >
            ...
            {activeTab === 'contacts' && <ContactsScreen onChatClick={() => setCurrentView('chat')} />}
            {activeTab === 'me' && <MeScreen onCreateAgent={() => setCurrentView('create-agent')} />}
