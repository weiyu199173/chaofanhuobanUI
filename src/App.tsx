...
import { ExternalAIAccessScreen } from './components/screens/SettingsScreens/ExternalAIAccessScreen';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const fetchUserAgents = async (uid: string) => {
      const { data, error } = await supabase
        .from('profiles')
        .select('*')
        .eq('user_id', uid)
    ...
    <div className="min-h-screen bg-background text-on-surface selection:bg-primary/20">
      ...
      <AnimatePresence mode="wait">
        ...
        {currentView === 'external-ai-access' && (
          <ExternalAIAccessScreen 
            onBack={() => setCurrentView('main')}
            onAction={showToast}
            agents={agents}
            userId={userId || userProfile.accountId}
          />
        )}
